...
> **For redesigning or replacing an existing site, use `/searchatlas:rebuild-website` instead.** That command consumes scout's output and focuses on design execution. This command is for greenfield only.
...
- Other commands (`/searchatlas:run-content`, `/searchatlas:run-pr`, `/searchatlas:run-seo`) all read the same BV downstream
...
**Do not check:** OTTO, PPC, LLM Visibility. These require a live crawlable site — `/searchatlas:run-seo` provisions them once the site has had time to be crawled.

Display brief check summary:

```
🏷️  Brand Vault   {emoji} {found / will create}
📍  GBP location  {emoji} {found / handled by /searchatlas:run-seo if missing}
```
...
(Captured here, used by /searchatlas:run-seo to size the post-launch cadence — PR, cloud
...
Budget tier
  {tier_name} ({range})
  → Captured for /searchatlas:run-seo (post-launch cadence sized there, not here)
...
What's next:
  /searchatlas:run-seo {domain}   — provisions OTTO, LLM Visibility, GBP linkage, and
                           sizes the ongoing cadence (cloud stacks, PRs,
                           articles, DPR) against the live, crawled site.
```

The publish workflow ends here. Real post-launch data (pillar scores, LLM tracking, rank tracking deltas) takes hours-to-days of crawl latency — synchronous post-launch provisioning would just create empty projects. `/searchatlas:run-seo` runs that cadence properly against a site that's been live long enough to be crawled.

---

## What this command does NOT do

- Touch an existing site — that's `/searchatlas:rebuild-website`
- Ingest scout output — also `/searchatlas:rebuild-website`
...
- Provision OTTO/PPC/LLMV/DPR or run any post-launch SEO cadence (cloud stacks, articles, PRs) — that's `/searchatlas:run-seo`'s job once the site is live and crawled
...
- `/searchatlas:onboard-client` — client setup; usually precedes this command
- `/searchatlas:rebuild-website` — for redesigning existing sites
- `/searchatlas:run-seo` — picks up once the site is live and crawled; provisions OTTO/LLMV/PPC and runs the ongoing cadence (cloud stacks, PRs, articles, DPR) sized by `budget-tier.json`

...
> **For brand new sites with no prior presence, use `/build-website` instead.** That command invents content from minimal seed input. This command transforms existing assets per operator-confirmed decisions.
...
Scout report is N days old. Two options:
  ...
  2. Re-run /scout first (recommended for accuracy)
...
For content-only updates, /run-content may be the right command instead.

  1. Yes, this is a rebuild — same style, different execution
  2. Switch to /run-content
...
Build loop similar to `/build-website` Phase 5, with rebuild-specific paths.
...
For each row in new-page-queue.csv:
  ...
  if action == "Merge":
    ...
    - Generate new copy
  if action == "New":
    - Standard greenfield generation (same as /build-website)
```
...
Same pattern as `/build-website` Phase 5 — every new/rewritten/redesigned page lives in Website Studio from the moment it's generated.
...
Post-launch ranking deltas can't be measured at T+0 — OTTO and KRT need crawl cycles. What we CAN do is lock a clean baseline against the OLD live site, so the operator's downstream `/run-seo` first-run can compute the actual delta once OTTO re-audits.
...
> **Why no OTTO issues scan against the new WS preview?** The preview isn't part of the OTTO crawl perimeter at publish time — running an issues summary there would return empty or stale data. OTTO's pillar delta is `/run-seo`'s job after the new site has had a 24–48h crawl cycle.

Output: `pre-launch-baseline.json` — baseline locked against the OLD site so the operator's downstream `/run-seo` first-run computes the actual delta once OTTO re-audits.
...
OTTO tracking on the new site requires the OTTO script. Website Studio publish embeds the SA tracking script automatically for SA-hosted domains — flagged if not detected. For external hosting, operator must install the OTTO script before fresh pillar scores are measurable in `/run-seo`.
...
Only claim what we can KNOW. Never claim "rankings improved" — that's `/run-seo`'s job after OTTO re-audits.
...
Pillar baseline    Tech {N}  Content {N}    Re-audit in /run-seo to see delta
...
> Workflow complete. `/run-seo` picks up the monthly cadence from here, and computes the actual ranking delta once OTTO re-audits.

No MCP calls in this phase. Anything that needs T+24h+ (indexing verification, post-launch rank snapshots, OTTO auto-fix sweeps on the new site) lives in the operator's downstream `/run-seo` first-run, not here.
...
- Build greenfield pages with no prior context — that's `/build-website`
...
**Workflow end-state:** site is live + before/after summary rendered. Actual post-launch ranking delta is `/run-seo`'s job — OTTO needs a 24–48h crawl cycle before fresh pillar scores and rank deltas are measurable.

## Related skills + commands

- `/scout` — upstream; produces the diagnosis this command consumes
- `/build-website` — for greenfield (this command's sibling)
- `/run-seo` — ongoing monthly cadence after Phase 7 handoff
- `/run-content` — for content refresh without design rebuild (smaller scope alternative)

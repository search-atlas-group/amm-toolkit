...
After the play executes, save a record to `${SA_CLIENTS_DIR:-$HOME/.searchatlas/clients}/{client_slug}/shots/play-{NN}-{YYYY-MM-DD}.md` (where `NN` is the play number zero-padded to 2 digits):
...
- IDs created/modified: {list}
- Next suggested: {next_command}
```

Also append a one-line entry to `${SA_CLIENTS_DIR:-$HOME/.searchatlas/clients}/{client_slug}/shots/shots.log`:

```
{ISO timestamp} · play {N} · {play_name} · {result_summary}
```

If `${SA_CLIENTS_DIR:-$HOME/.searchatlas/clients}/{client_slug}/shots/` doesn't exist yet, create it first.
